      ...
      {props.chains.map((item: Chain, index: number) => {
        return (
          <li key={index}>
            {index !== 0 && (
              <img
                src={require(`../../assets/logo/${item.chain_id}.png`)}
                alt=""
              />
            )}
          </li>
...
const columns: TableProps<DataType>["columns"] = [
  ...
  },
  {
    ...
    render: (_) => (
      <div className="icon-box">
        ...
        <p className="bold-text flex-c">
          {_.description}
          <span>{_.currency_name}</span>
        </p>
      </div>
    ),
  },
      ...
      <div className="chains-list">
        {_.chain_info.length <= 2 ? (
          ...
          <div className="more-logo">
            <img
              src={require(`../../assets/logo/${_.chain_info[0].chain_id}.png`)}
              alt=""
            />
            <Popover
              placement="right"
              title={null}
              content={<MoreChainPop chains={_.chain_info} />}
            >
  ...
  },
  {
    ...
    render: (text) => (
      <p className={`${text > 0 ? "up-c" : "down-c"}`}>
...
const MemesView = (): ReactElement<ReactNode> => {
  ...
  const getMemesList = async () => {
    if (state.memes) {
      setData(JSON.parse(state.memes));
      return;
    }
    setLoading(true);
    ...
    <div className="memes-view">
      <div className="memes-inner">
        <div className="mask-box">
          <img
            src={require("../../assets/images/memes_mask.png")}
            className="left-mask"
            alt=""
          />
        </div>
