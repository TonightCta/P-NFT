...
export const NFTAddress: string =
  LAND === "taiko"
    ...
    : MODE === "production"
    ? Address.PlianContractAddress721Main
    : Address.PlianContractAddress721Test;
export const MarketAddress: string =
  LAND === "taiko"
    ? MODE === "taikomain"
      ? Address.TaikoContractAddressMarketMain
      : Address.TaikoContractAddressMarketTest
    : MODE === "production"
    ? Address.PlianContractAddressMarketMain
    : Address.PlianContractAddressMarketTest;
...
export const useContract = () => {
  ...
  const mint = async (_data_ipfs: string) => {
    ...
    const Fee: string = web3V2.utils.toWei(
      FilterAddressToName(state.chain as string).fee,
      "ether"
    );
    const Gas: string = FilterAddressToName(state.chain as string).gas;
    return new Promise((resolve, reject) => {
      NFTContract.methods
        .mint(state.address, _data_ipfs)
        .send({
          from: send.from,
          gas: Gas,
          value: Fee,
  ...
  const tokenMint = async (_data_ipfs: string, _pay_token_address: string) => {
    ...
    return new Promise(async (resolve, reject) => {
      const MintV2Contract = new web3V2.eth.Contract(
        NER721ABI as any,
      ...
      const Gas: string = FilterAddressToName(state.chain as string).gas;
      const fee = await MintV2Contract.methods
        .mintFees(_pay_token_address)
        .call();
      const pay_address =
        _pay_token_address.slice(
          _pay_token_address.length - 8,
          _pay_token_address.length
        ) === "00000000";
      MintV2Contract.methods
        .mintToken(state.address, _pay_token_address, _data_ipfs)
        .send({
          from: send.from,
          gas: Gas,
    ...
    const getInfo = async (index: number) => {
      try {
        const id = await NFTContract.methods
          .tokenOfOwnerByIndex(send.from, index)
          .call();
  ...
  const putApprove = async (
    ...
    return new Promise(async (resolve, reject) => {
      ContractV2.methods
        .approve(MarketAddress, _token_id)
        .send(
          LAND === "taiko"
            ? {
                from: send.from,
  ...
  const queryMint = async () => {
    ...
    const SBTContractInner = new web3V2.eth.Contract(
      ABISBT as any,
      Address.PlianContractSBTTest,
      {
        // gasPrice: gasPrice,
  ...
  const officalTotalSupply = async (): Promise<number> => {
    ...
    const NFTAddress =
      LAND === "taiko"
        ...
        ? FilterAddress(state.chain as string).contract_721
        : FilterAddress(state.chain as string).contract_721_test;
    const NFTContractInner = new web3V2.eth.Contract(
      ABI721 as any,
      NFTAddress,
      {
        gasPrice: gasPrice,
  ...
  const transHash = async (
    ...
    const contract = new web3P.eth.Contract(
      HASHABI as any,
      "0x60D8b4198E78ee47a27d12B0D188C56578824875"
    );
    //@ts-ignore
    const hash: string = await contract.methods
      .hashTransaction(state.address, +_amount, _nonce)
      .call();
    return hash;
  };
      ...
      Consideration: [
        {
          ...
          StartAmount: String(
            web3V2.utils.toWei(String((_amount * 0.975).toFixed(4)), "ether")
          ),
          EndAmount: String(
            web3V2.utils.toWei(String((_amount * 0.975).toFixed(4)), "ether")
          ),
        ...
        },
        {
          ...
          IdentifierOrCriteria: "0",
          StartAmount: String(
            web3V2.utils.toWei(String((_amount * 0.025).toFixed(4)), "ether")
          ),
          EndAmount: String(
            web3V2.utils.toWei(String((_amount * 0.025).toFixed(4)), "ether")
          ),
  ...
  const OPBuy = (_amount: string, _key: string) => {
    const Gas: string = FilterAddressToName(state.chain as string).gas;
    const OPContract = new web3V2.eth.Contract(
      OPABI as any,
    ...
    return new Promise((resolve, reject) => {
      OPContract.methods
        .fulfillAdvancedOrder(_amount, 1, 1, _key, send.from)
        .send({
          value: _amount,
          from: send.from,
          gas: Gas,
  ...
  const CreateCollectionWith404 = (
    _name: string,
    _symbol: string,
    _token_uri: string,
    _decimals: number,
    _total_supply: number,
    _address: string
  ) => {
    const Gas: string = FilterAddressToName(state.chain as string).gas;
    const Contract = new web3V2.eth.Contract(
      CollectionABI,
  ...
  const QueryNFTMsg = async (_address: string) => {
    const web3 = new Web3(
      (state.wallet === "metamask" && provider) ||
        (state.wallet === "coinbase" && ethereumCoinbase) ||
        (state.wallet === "walletconnect" && walletProvider) ||
    ...
    const priceChain = await Contract.methods.NFTPrice().call();
    const price = web3.utils.fromWei(priceChain, "ether");
